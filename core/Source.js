...
export class Source extends Emitter {
    session;
    _stacks;
    constructor(session, stacks = [], { autoInit = true } = {}) {
        super();
    ...
    get tokens() { return this.session.state.source?.tokens ?? []; }
    get burned() { return this.session.state.source?.burned ?? []; }
    get seed() { return this.session.state.source?.seed ?? null; }
    get policy() {
