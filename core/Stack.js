...
export class Stack extends Emitter {
     ...
     */
    constructor(session, tokens = [], { seed, autoInit = true } = {}) {
        super();
        if (!session) {
    ...
    get size() { return this.session.state.stack?.stack.length ?? 0; }
    get tokens() { return this.session.state.stack?.stack ?? []; }
    get drawn() { return this.session.state.stack?.drawn ?? []; }
    get discards() { return this.session.state.stack?.discards ?? []; }
    /**
    ...
    _drawSingle() {
        ...
        this.session.change("draw card", (doc) => {
            if (!doc.stack || doc.stack.stack.length === 0)
                return;
    ...
    insertAt(card, index) {
        ...
        this.session.change("insert card", (doc) => {
            ...
            if (idx < 0)
                idx = 0;
            if (idx > doc.stack.stack.length)
                idx = doc.stack.stack.length;
    ...
    removeAt(index) {
        ...
        this.session.change("remove card at", (doc) => {
            if (!doc.stack)
                return;
            if (index < 0 || index >= doc.stack.stack.length)
                return;
            const [itemProxy] = doc.stack.stack.splice(index, 1);
            removed = this._clone(itemProxy);
        });
        if (removed)
            this.emit("stack:remove", { payload: { card: removed, index } });
        return removed;
    }
    /**
    ...
    swap(i, j) {
        ...
        this.session.change("swap cards", (doc) => {
            if (!doc.stack)
                return;
            const stack = this._clone(doc.stack.stack);
            if (i < 0 || j < 0 || i >= stack.length || j >= stack.length)
                return;
            const temp = stack[i];
            stack[i] = stack[j];
            stack[j] = temp;
