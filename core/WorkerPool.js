...
export class WorkerPool extends EventEmitter {
     ...
     */
    async executeParallel(type, dataArray) {
        const promises = dataArray.map(data => this.execute(type, data));
        return Promise.all(promises);
    }
    /**
     * Execute tasks in batches
     */
    async executeBatched(type, dataArray, batchSize) {
        ...
        for (let i = 0; i < dataArray.length; i += actualBatchSize) {
            ...
            const batchResults = await this.executeParallel(type, batch);
    ...
    async shutdown() {
        ...
        const terminatePromises = Array.from(this.workers.values()).map(workerInfo => workerInfo.worker.terminate());
