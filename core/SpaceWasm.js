...
export class SpaceWasm extends Emitter {
    ...
    _syncToChronicle() {
        ...
        try {
            ...
            // WASM: { zones: { zoneName: { placements: [...], lock: {...} } } }
            // Chronicle: { zones: { zoneName: [...] } }
            const chronicleZones = {};
            if (wasmState.zones) {
                for (const [zoneName, zone] of Object.entries(wasmState.zones)) {
                    chronicleZones[zoneName] = zone.placements || [];
                }
            }
    ...
    place(zoneName, token, opts = {}) {
        ...
        if (this._wasmSpace && isWasmAvailable()) {
            try {
                ...
                const placementJson = this._wasmSpace.place(zoneName, tokenJson, opts.x ?? 0, opts.y ?? 0);
                const placement = JSON.parse(placementJson);
                // Override with any custom opts
                if (opts.faceUp !== undefined)
                    placement.faceUp = opts.faceUp;
                if (opts.label !== undefined)
                    placement.label = opts.label;
