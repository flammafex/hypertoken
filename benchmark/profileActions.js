 ...
 */
function wrapEngineWithProfiler(engine, profiler) {
    const originalDispatch = engine.dispatch.bind(engine);
    engine.dispatch = function (type, payload = {}, opts = {}) {
...
async function runBenchmark() {
    ...
    for (let i = 0; i < 500; i++) {
        ...
        if (i % 20 === 0)
            engine.dispatch('stack:burn', { count: 2 });
        // Advanced operations
        if (i % 30 === 0)
            engine.dispatch('stack:cut', { position: 26 });
        if (i % 40 === 0)
            engine.dispatch('stack:reverse', { start: 0, end: 10 });
        if (i % 50 === 0)
            engine.dispatch('stack:peek', { count: 5 });
        // Reset periodically
        if (i % 100 === 0)
    ...
    for (let i = 0; i < 200; i++) {
        ...
        if (i % 20 === 0)
            engine.dispatch('source:draw', { count: 5 });
        if (i % 30 === 0)
    ...
    for (let i = 0; i < 100; i++) {
        if (i % 10 === 0) {
            engine.dispatch('batch:filterTokens', {
                tokens,
                predicate: (t) => parseInt(t.id.split('-')[1]) % 2 === 0,
            });
        }
        if (i % 20 === 0) {
            engine.dispatch('batch:transformTokens', {
                tokens: tokens.slice(0, 20),
                operation: (t) => ({ ...t, modified: true }),
            });
        }
    }
