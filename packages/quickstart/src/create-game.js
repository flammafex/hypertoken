...
async function generateProject(name, template, agentName) {
    ...
    await writeFile(join(name, 'package.json'), generatePackageJson(name));
    await writeFile(join(name, 'tsconfig.json'), generateTsConfig());
    await writeFile(join(name, 'README.md'), generateReadme(name, template));
    await writeFile(join(name, 'src', 'game.ts'), generateGameFile(template, agentName));
    await writeFile(join(name, 'demo', 'cli.ts'), generateCliFile(template));
    await writeFile(join(name, 'tokens', 'tokens.json'), generateTokensFile(template));
    await writeFile(join(name, '.gitignore'), 'node_modules/\ndist/\n*.log\n.DS_Store\n');
}
...
function generateGameFile(template, agentName) {
    if (template === 'card') {
...
function generateCardGameTemplate(agentName) {
...
function generateBoardGameTemplate(agentName) {
...
function generateResourceGameTemplate(agentName) {
...
function generateCustomTemplate(agentName) {
...
function generateCliFile(template) {
...
function generateTokensFile(template) {
    if (template === 'card') {
        return JSON.stringify({
            tokens: [
                {
