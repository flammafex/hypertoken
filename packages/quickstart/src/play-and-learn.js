...
export async function playAndLearn(isJoining) {
...
async function runNetworkedTicTacToe(ws, player, hasOpponent) {
    await runTicTacToeDemo(player, ws, hasOpponent);
}
async function runTicTacToeDemo(player, ws, hasOpponent) {
    ...
    while (!gameOver) {
        if (currentPlayer === player) {
            const { position } = await prompts({
                ...
                validate: (value) => {
                    if (value < 1 || value > 9)
                        return 'Position must be between 1 and 9';
            ...
            if (hasOpponent && ws) {
                ...
                const opponentMove = await new Promise((resolve) => {
                    const handler = (data) => {
