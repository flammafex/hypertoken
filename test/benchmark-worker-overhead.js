...
async function measureLatency(fn, iterations) {
    const latencies = [];
    // Warmup
    for (let i = 0; i < 3; i++) {
        await fn();
...
async function benchmarkWorkerOverhead() {
    ...
    await worker.init();
    const pingLatencies = await measureLatency(async () => {
        await worker.ping();
    }, iterations);
    ...
    console.log('-'.repeat(80));
    const actionLatencies = await measureLatency(async () => {
    ...
    console.log('-'.repeat(80));
    const burstLatencies = await measureLatency(async () => {
        await Promise.all([
