...
await test('Policy initializes correctly', () => {
    const policy = new Policy('test-policy', (engine) => true, (engine) => { });
...
await test('Policy initializes with options', () => {
    const policy = new Policy('test', () => true, () => { }, { priority: 10, once: true, enabled: false });
...
await test('Policy evaluates condition and executes effect', () => {
    ...
    const policy = new Policy('test', (eng) => eng._agents.length > 0, (eng) => {
        effectExecuted = true;
    });
    ...
    engine._agents.push({ name: 'Alice', id: '1', resources: {} });
    policy.evaluate(engine);
...
await test('Policy handles condition errors gracefully', () => {
    ...
    const policy = new Policy('broken-condition', () => {
...
await test('Policy handles effect errors gracefully', () => {
    ...
    const policy = new Policy('broken-effect', () => true, () => {
...
await test('Multiple policies with different priorities', () => {
    ...
    const policy1 = new Policy('low', () => true, () => {
        executionOrder.push('low');
    }, { priority: 1 });
    const policy2 = new Policy('high', () => true, () => {
        executionOrder.push('high');
    }, { priority: 10 });
    const policy3 = new Policy('medium', () => true, () => {
...
await test('Policy conditions can depend on game state', () => {
    ...
    const scorePolicy = new Policy('win-check', (eng) => (eng._gameState.score || 0) >= 100, (eng) => {
...
await test('Policy effects can modify engine state', () => {
    ...
    const roundPolicy = new Policy('advance-round', (eng) => eng._agents.length >= 2, (eng) => {
    ...
    // Add agents
    engine._agents.push({ name: 'Alice' }, { name: 'Bob' });
    roundPolicy.evaluate(engine);
...
await test('Game end policy with multiple conditions', () => {
    ...
    engine._agents.push({ name: 'Alice', resources: { score: 0 } });
    const gameEndPolicy = new Policy('game-end', (eng) => {
...
await test('Resource management policy', () => {
    const engine = new Engine();
    engine._agents.push({ name: 'Alice', resources: { gold: 50 } }, { name: 'Bob', resources: { gold: 30 } });
    const taxPolicy = new Policy('tax-collection', (eng) => (eng._gameState.turn ?? 0) % 5 === 0, (eng) => {
        eng._agents.forEach((agent) => {
...
await test('Policy toJSON() exports state', () => {
    const policy = new Policy('test-policy', () => true, () => { }, { priority: 5, once: true });
    policy._hits = 3;
    policy._fired = true;
