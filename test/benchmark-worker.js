...
async function runBenchmark(engine, mode, testName, iterations, actionFn) {
    const times = [];
    // Warmup
    for (let i = 0; i < 3; i++) {
...
function printComparison(workerResult, nonWorkerResult) {
...
async function benchmarkWorker() {
    ...
    const workerResult1 = await runBenchmark(workerEngine1, 'worker', 'Single shuffle', iterations, async (engine) => {
        await engine.dispatchAsync('stack:shuffle', { seed: Math.random() });
    });
    const nonWorkerResult1 = await runBenchmark(nonWorkerEngine1, 'non-worker', 'Single shuffle', iterations, async (engine) => {
    ...
    const workerResult2 = await runBenchmark(workerEngine2, 'worker', 'Draw 5 cards', iterations, async (engine) => {
    ...
    const nonWorkerResult2 = await runBenchmark(nonWorkerEngine2, 'non-worker', 'Draw 5 cards', iterations, async (engine) => {
    ...
    const workerResult3 = await runBenchmark(workerEngine3, 'worker', 'Concurrent shuffles', 20, // Fewer iterations for concurrent test
    async (engine) => {
        await Promise.all([
    ...
    const nonWorkerResult3 = await runBenchmark(nonWorkerEngine3, 'non-worker', 'Concurrent shuffles', 20, async (engine) => {
    ...
    await new Promise(resolve => setTimeout(resolve, 500));
    const batchResult = await runBenchmark(workerBatchEngine, 'worker', 'Batched actions', 50, async (engine) => {
        await Promise.all([
    ...
    const noBatchResult = await runBenchmark(workerNoBatchEngine, 'worker', 'Non-batched actions', 50, async (engine) => {
        await Promise.all([
            engine.dispatchAsync('stack:shuffle'),
            engine.dispatchAsync('stack:draw', { count: 1 }),
            engine.dispatchAsync('stack:shuffle'),
