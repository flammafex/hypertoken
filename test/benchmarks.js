...
// Engine Benchmarks
// ============================================================
function benchmarkEngine(runner) {
...
// Token Benchmarks
// ============================================================
function benchmarkTokens(runner) {
...
// Stack Benchmarks
// ============================================================
function benchmarkStack(runner) {
...
// Space Benchmarks
// ============================================================
function benchmarkSpace(runner) {
...
// ============================================================
function benchmarkBatchOperations(runner) {
    ...
    runner.benchmark('Batch: Filter 100 tokens', () => {
        ...
        BatchActions['tokens:filter'](engine, {
            tokens,
            predicate: (token) => token.meta.value > 50
        });
    }, 1000);
    ...
    runner.benchmark('Batch: Find in 100 tokens', () => {
        ...
        BatchActions['tokens:find'](engine, {
            source: 'stack',
            predicate: (token) => token.meta.value > 90
        });
    }, 100); // Reduced to avoid CRDT overload
}
// ============================================================
...
// ============================================================
function benchmarkLargeScale(runner) {
...
// ============================================================
function benchmarkRealWorld(runner) {
    ...
    runner.benchmark('Scenario: Complete card game round (4 players, 5 cards each)', () => {
        ...
        for (let player = 0; player < 4; player++) {
            ...
            if (cards) {
                cards.forEach((card) => space.place(`player${player}`, card));
            }
        }
    ...
    runner.benchmark('Scenario: Filter and modify 200 tokens', () => {
        ...
        const filtered = BatchActions['tokens:filter'](engine, {
            tokens,
            predicate: (token) => token.meta.value > 30
        });
        BatchActions['tokens:forEach'](engine, {
            tokens: filtered,
            operation: (token) => {
