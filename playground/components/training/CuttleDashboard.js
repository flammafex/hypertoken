 ...
 * CuttleDashboard.js
 *
 * Specialized training dashboard for Cuttle.
 * Focuses on Board Presence, Point Progress (to 21), and Scuttle efficiency.
 */
...
export function CuttleDashboard(props) {
  return html`
    <${DashboardWrapper} title="Cuttle Engine Monitor" ...${props}>
      ${({ stats, session, actionLabels, setShowEpisodeDetail, replayEpisode }) => {
        if (!stats) return html`
          <div class="empty-state">
            <div class="empty-icon">⚔️</div>
            <p>Monitor the agent's point-building and disruption strategies.</p>
          </div>
        `;
            ...
            <div class="stats-column">
              <div class="stats-panel">
                ...
                <div class="stats-grid">
                  ...
                  <${StatItem} label="Avg Points" value=${stats.avgScore ? stats.avgScore.toFixed(1) : '--'} />
                  <${StatItem} label="Win Rate" value="${(stats.winRate * 100).toFixed(1)}%" />
                  <${StatItem} label="Win/Step Ratio" value="${(stats.avgSteps > 0 ? (stats.winRate / stats.avgSteps * 100) : 0).toFixed(2)}" />
                </div>
              </div>
            </div>
          </div>
