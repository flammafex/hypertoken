...
export class GameLoop extends Emitter {
    ...
    // --- STATE GETTERS ---
    get turn() { return this.session.state.gameLoop?.turn ?? 0; }
    get running() { return this.session.state.gameLoop?.running ?? false; }
    get activeAgentIndex() { return this.session.state.gameLoop?.activeAgentIndex ?? -1; }
    get phase() { return this.session.state.gameLoop?.phase ?? "setup"; }
    get maxTurns() { return this.session.state.gameLoop?.maxTurns ?? Infinity; }
    set maxTurns(value) {
        this.session.change("set maxTurns", (doc) => {
            if (doc.gameLoop)
                doc.gameLoop.maxTurns = value;
        });
    }
    // --- REACTIVE SYNC ---
    _syncState() {
    ...
    nextTurn() {
        this.session.change("next turn", (doc) => {
            if (!doc.gameLoop || !this.engine._agents.length)
                return;
            doc.gameLoop.turn++;
