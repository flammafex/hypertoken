 ...
 */
// ./interface/OpenAIAgent.ts

/** Minimal OpenAI client interface — compatible with the official `openai` SDK. */
...
export class OpenAIAgent {
  private api: OpenAIApiClient;
  model: string;
  systemPrompt: string;
  temperature: number;

  constructor(apiClient: OpenAIApiClient, {
    model = "gpt-4o-mini",
  ...
  async think(
    engine: { describe(): unknown; availableActions?(): unknown[] },
    agent: { name: string },
